...
power_my = zeros(1,length(N_range));
power_my_1 = zeros(1,length(N_range));
power_my_2 = zeros(1,length(N_range));
power_my_3 = zeros(1,length(N_range));
...
for sim = 1:N_sim
    ...
    for N_index = 1:length(N_range)
        ...
        power_my(N_index) = power_my(N_index) + 10*log10(1000*p_my(end)/omega^K);

        [X_my_1,theta_my_1,p_my_1] = getX_my_PM_b(H_au,H_ar,H_ru,SNR,B1,Nmax,res_th,omega);
        power_my_1(N_index) = power_my_1(N_index) + 10*log10(1000*p_my_1(end)/omega^K);

        [X_my_2,theta_my_2,p_my_2] = getX_my_PM_b(H_au,H_ar,H_ru,SNR,B2,Nmax,res_th,omega);
        power_my_2(N_index) = power_my_2(N_index) + 10*log10(1000*p_my_2(end)/omega^K);

        [X_my_3,theta_my_3,p_my_3] = getX_my_PM_b(H_au,H_ar,H_ru,SNR,B3,Nmax,res_th,omega);
        power_my_3(N_index) = power_my_3(N_index) + 10*log10(1000*p_my_3(end)/omega^K);
...
figure
plot(N_range,power_my,'-o','color',[1,0,0],'LineWidth',1.5, 'markersize',8)
hold on
plot(N_range,power_my_1,'-^','color',[0,0,1],'LineWidth',1.5)
plot(N_range,power_my_2,'-*','color',[0,0,0],'LineWidth',1.5)
plot(N_range,power_my_3,'-+','color',[0.5,0.5,0],'LineWidth',1.5)
hold off
xlabel('Number of reflecting elements');
ylabel('Average transmit power (dBm)');
grid on
legend('Proposed w/ IRS','Proposed, B = 1','Proposed, B = 2', 'Proposed, B = 3');
